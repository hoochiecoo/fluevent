...
class MainActivity : FlutterActivity() {
    ...
    private fun processImageProxyWithOpenCV(imageProxy: ImageProxy) {
        ...
        // 1. Convert ImageProxy (YUV) to OpenCV Mat (RGBA)
        ...
        val blurredMat = Mat()
        Imgproc.GaussianBlur(grayMat, blurredMat, org.opencv.core.Size(kernelSize.toDouble(), kernelSize.toDouble()), 0.0)
        ...
        val lines = Mat()
        Imgproc.HoughLinesP(edgesMat, lines, 1.0, Math.PI / 180, houghThreshold, houghMinLineLength, houghMaxLineGap)
        
        val lineCount = lines.rows()
        runOnUiThread {
            eventSink?.success("Линий найдено: $lineCount")
        }
